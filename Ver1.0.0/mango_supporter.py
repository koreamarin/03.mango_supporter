...
import sys, os, time, requests, webbrowser, pyautogui, threading, getmac, subprocess
...
class MainDialog(QDialog) :
    def __init__(self) :
        ...
        self.mapping_ID_edit.setText('')
        self.mapping_PW_edit.setText('')
        self.client_id_edit.setText('')
        self.client_sr_edit.setText('')
        self.translation_word = None
        self.tree_radioButton.setChecked(True)
        self.first_category_radioButton.setChecked(True)
        self.themango_search_radioButton.setChecked(True)
        ...
        # 검색/키워드입력 버튼 (셀러라이프 자동검색/입력 함수 함수 실행)
        ...
        self.Alignmenter_ID_edit.returnPressed.connect(self.Alignmenter_login)
        self.Alignmenter_PW_edit.returnPressed.connect(self.Alignmenter_login)
...
########################매핑 서포터 함수###################################
    ...
    def mapping_login(self) :
        ...
        tmg_num = self.mapping_decry_tmg_status.text()
        ID = self.mapping_ID_edit.text()
        PW = self.mapping_PW_edit.text()
        ...
        try :
            ...
            self.mapping_status_label_2.setText('크롬 베타버전이 선택되지 않았습니다.')
            self.mapping_status_label_3.setText('')
            return 0
            
        try :    
            # 크롬 드라이버 104 버전 열기, 베타버전
            self.mapping_driver = webdriver.Chrome(rf'{self.chrome_driver_fname}', chrome_options=chrome_options)

        except :
            self.mapping_status_label_1.setText('')
            self.mapping_status_label_2.setText('같은 폴더내에 크롬드라이버 version 104 (베타버전)이 없습니다.')
            self.mapping_status_label_3.setText('')
            return 0
        ...
        # 브라우저 접속에 성공하였다면 크롬 위치정보 저장
        self.file_path_info_save()
        ...
        # 아이디 입력창
        id = self.mapping_driver.find_element(By.CSS_SELECTOR, "body > table:nth-child(2) > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td:nth-child(2) > input")                # driver내에서 CSS 선택자 "#id"로 아이디 입력창을 찾은 후, 그 정보를 id라는 변수에 담는다.
        id.click()                                                                      # 찾은 아이디 입력창을 클릭 --> 사실 클릭안하고 바로 send_keys 써도 됨, 하지만 네이버처럼 복붙 필요한곳은 클릭 해줘야 함.
        id.send_keys(ID)                                                                # 아이디 입력

        # 비밀번호 입력창
        pw = self.mapping_driver.find_element(By.CSS_SELECTOR, "body > table:nth-child(2) > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td:nth-child(4) > input")                # driver내에서 CSS 선택자 "#id"로 아이디 입력창을 찾은 후, 그 정보를 id라는 변수에 담는다.
        pw.click()                                                                      # 찾은 아이디 입력창을 클릭 --> 사실 클릭안하고 바로 send_keys 써도 됨
        pw.send_keys(PW)                                                                # 비밀번호 입력

        # 로그인 버튼 클릭
        self.mapping_driver.find_element(By.CSS_SELECTOR, "body > table:nth-child(2) > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td:nth-child(5) > div > a > span").click()
        time.sleep(1)

        try : 
            if self.tree_radioButton.isChecked() == 1 :
                # 카테고리 트리형으로 이동
                self.mapping_driver.get(f"https://tmg{tmg_num}.cafe24.com/mall/admin/admin_category.php")

            if self.step_radioButton.isChecked() == 1 :
                # 카테고리 단계형으로 이동
                self.mapping_driver.get(f"https://tmg{tmg_num}.cafe24.com/mall/admin/admin_category_new.php")

        except : 
            self.mapping_status_label_2.setText("로그인 실패")
    ...
    def category_load(self) :
        ...
        try :
            ...
            self.mapping_status_label_1.setText('카테고리탭 인식 실패')
    ...
    def sellerlife_search(self) :
        ...
        try : 
            if self.sellerlife_search_edit.text() == '' :
                ...
                # 브라우저 꺼짐 방지
                hide_chrome_options = Options()
                hide_chrome_options.add_experimental_option("detach", True)

                # 백그라운드에서 드라이버 실행
                hide_chrome_options.add_argument("headless")

                # 불필요한 에러 메시지 없애기 (꼭 필요한 코드는 아님. 없어도됨. 다만 이상한 오류코드가 발생하기에 그거 없애려고 추가하는 것)
                hide_chrome_options.add_experimental_option("excludeSwitches", ["enable-logging"])


                # 크롬 베타버전으로 열기, 베타버전
                hide_chrome_options.binary_location = f'{self.chrome_fname}'

                # 크롬 드라이버 104 버전 열기, 베타버전
                hide_driver = webdriver.Chrome(rf'{self.chrome_driver_fname}', chrome_options=hide_chrome_options)

                # 웹페이지가 로딩 될때까지 5초는 기다림
                hide_driver.implicitly_wait(5)

                # 셀러라이프로 이동
                hide_driver.get(f"https://sellerlife.co.kr/keyword")

                # 검색창 찾기
                search_window = hide_driver.find_element(By.CSS_SELECTOR, ".form-control.fw-bold")

                # 검색창에 sellerlife_search_edit의 검색어 입력
                search_window.send_keys(self.sellerlife_search_edit.text())

                # 검색 버튼 클릭
                hide_driver.find_element(By.CSS_SELECTOR, "#keyword-search-btn").click()
                time.sleep(1)

                # 키워드 추천 버튼 클릭
                hide_driver.find_element(By.CSS_SELECTOR, "#keyword-recommend").click()

                # 40개 추천 버튼 클릭
                hide_driver.find_element(By.CSS_SELECTOR, "#select-keyword > div > div:nth-child(1) > div > ul > li:nth-child(3) > a").click()

                keyword = hide_driver.find_element(By.CSS_SELECTOR, ".form-control.p-3").get_attribute('value')
            ...
            self.keyword_amount_label.setText('셀러라이프 검색 실패 [크롬브라우저 확인]')
    ...
    def mapping_guardian(self) :
        ...
        if tmg_num != extracted_tmg_num :
            self.mapping_status_label_1.setText('로그인한 tmg 번호와 사이트에서 접근한 tmg 번호가 다릅니다.')
            self.mapping_status_label_2.setText('비정상적인 접근으로 크롬을 종료합니다.')
...
##################################자동 창 정렬 함수################################################
    ...
    def Alignmenter_login(self) :
        ...
        # tmg번호, 아이디, 비밀번호 불러와서 변수에 저장하는 코드.
        tmg_num = self.alignmenter_decry_tmg_status.text()
        ID = self.Alignmenter_ID_edit.text()
        PW = self.Alignmenter_PW_edit.text()
        ...
        try :
            ...
            self.alignmenter_status_label_2.setText('크롬 베타버전이 선택되지 않았습니다.')
            self.alignmenter_status_label_3.setText('')
            return 0
            
        try :    
            # 크롬 드라이버 104 버전 열기, 베타버전
            self.alignmenter_driver = webdriver.Chrome(rf'{self.chrome_driver_fname}', chrome_options=chrome_options)

        except :
            self.alignmenter_status_label_1.setText('')
            self.alignmenter_status_label_2.setText('같은 폴더내에 크롬드라이버 version 104 (베타버전)이 없습니다.')
            self.alignmenter_status_label_3.setText('')
            return 0
        ...
        try : 
            ...
            self.alignmenter_driver.get('https://www.coupang.com/')
            ...
            # 아이디 입력창
            id = self.alignmenter_driver.find_element(By.CSS_SELECTOR, "body > table:nth-child(2) > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td:nth-child(2) > input")                # driver내에서 CSS 선택자 "#id"로 아이디 입력창을 찾은 후, 그 정보를 id라는 변수에 담는다.
            id.click()                                                                      # 찾은 아이디 입력창을 클릭 --> 사실 클릭안하고 바로 send_keys 써도 됨, 하지만 네이버처럼 복붙 필요한곳은 클릭 해줘야 함.
            id.send_keys(ID)                                                                # 아이디 입력

            # 비밀번호 입력창
            pw = self.alignmenter_driver.find_element(By.CSS_SELECTOR, "body > table:nth-child(2) > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td:nth-child(4) > input")                # driver내에서 CSS 선택자 "#id"로 아이디 입력창을 찾은 후, 그 정보를 id라는 변수에 담는다.
            pw.click()                                                                      # 찾은 아이디 입력창을 클릭 --> 사실 클릭안하고 바로 send_keys 써도 됨
            pw.send_keys(PW)                                                                # 비밀번호 입력

            # 로그인 버튼 클릭
            self.alignmenter_driver.find_element(By.CSS_SELECTOR, "body > table:nth-child(2) > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td:nth-child(5) > div > a > span").click()
            time.sleep(1)

            # 카테고리 트리형으로 이동
            self.alignmenter_driver.get(f"https://tmg{tmg_num}.cafe24.com/mall/admin/admin_goods_update.php?s_market=%5B%22AUC%22%2C%22GMK%22%2C%22INTER%22%2C%22WMP%22%2C%22TMON%22%2C%22LTON%22%2C%22SSG%22%5D")

        except : 
            self.alignmenter_status_label_2.setText("로그인 실패")

            # 새 탭 닫기
            self.alignmenter_driver.close()
    ...
    def alignmenter_guardian(self) :
        ...
        if B_list[0] == 'https://tm' : 
            ...
            if tmg_num != extracted_tmg_num :
                self.alignmenter_status_label_1.setText('로그인한 tmg 번호와 사이트에서 접근한 tmg 번호가 다릅니다.')
                self.alignmenter_status_label_2.setText('비정상적인 접근으로 크롬을 종료합니다.')
